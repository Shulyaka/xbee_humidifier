...
class Commands:
  ...
  def __init__(self, tosr_switch, tosr_temp, humidifier, humidifier_sensor, humidifier_available, humidifier_switch):
  ...
  def cmd_logger_set_target(self, sender_eui64, target=None, name=None):
    if name:
      ...
      logging.getLogger("__main__").set_default_target(sender_eui64 if target is None else target)
  ...
  def cmd_humidifier_get_state(self, sender_eui64, number):
    return {"name": self._humidifier[number].name, "available": self._humidifier[number].available, "is_on": self._humidifier[number].is_on, "working": self._tosr_switch.state, "device_class": self._humidifier[number].device_class, "capability_attributes": self._humidifier[number].capability_attributes, "state_attributes": self._humidifier[number].state_attributes, "extra_state_attributes": self._humidifier[number].extra_state_attributes}
  ...
  def cmd_bind_humidifier_available(self, sender_eui64, number):
    if number not in self._humidifier_available_binds and number in self._humidifier_available:
      self._humidifier_available_binds[number] = {}
    if sender_eui64 not in self._humidifier_available_binds[number]:
      self._humidifier_available_binds[number][sender_eui64] = self._humidifier_available[number].subscribe(lambda x: transmit(sender_eui64, json_dumps({"name": self._humidifier[number].name, "available": x})))
    ...
    if sender_eui64 is None:
      if number in self._humidifier_available_binds:
        for unsubscribe in self._humidifier_available_binds[number]:
          unsubscribe()
        self._humidifier_available_binds[number] = {}
    elif number in self._humidifier_available_binds and sender_eui64 in self._humidifier_available_binds[number]:
      self._humidifier_available_binds[number].pop(sender_eui64)()

  def cmd_bind_humidifier_working(self, sender_eui64, number):
    if number not in self._humidifier_switch_binds and number in self._humidifier_switch:
      self._humidifier_switch_binds[number] = {}
    if sender_eui64 not in self._humidifier_switch_binds[number]:
      self._humidifier_switch_binds[number][sender_eui64] = self._humidifier_switch[number].subscribe(lambda x: transmit(sender_eui64, json_dumps({"name": self._humidifier[number].name, "working": x})))
    ...
    if sender_eui64 is None:
      if number in self._humidifier_switch_binds:
        for unsubscribe in self._humidifier_switch_binds[number]:
          unsubscribe()
        self._humidifier_switch_binds[number] = {}
    elif number in self._humidifier_switch_binds and sender_eui64 in self._humidifier_switch_binds[number]:
  ...
  def cmd_bind_tosr0x_relay(self, sender_eui64, switch_number):
    if switch_number not in self._tosr0x_relay_binds and switch_number in self._tosr_switch:
      self._tosr0x_relay_binds[switch_number] = {}
    if sender_eui64 not in self._tosr0x_relay_binds[switch_number]:
      self._tosr0x_relay_binds[switch_number][sender_eui64] = self._tosr_switch[switch_number].subscribe(lambda x: transmit(sender_eui64, json_dumps({"tosr0x_relay_" + str(switch_number): x})))
    ...
    if sender_eui64 is None:
      if switch_number in self._tosr0x_relay_binds:
        for unsubscribe in self._tosr0x_relay_binds[switch_number]:
          unsubscribe()
        self._tosr0x_relay_binds[switch_number] = {}
    elif switch_number in self._tosr0x_relay_binds and sender_eui64 in self._tosr0x_relay_binds[switch_number]:
  ...
  def cmd_bind_tosr0x_temp(self, sender_eui64):
    if sender_eui64 not in self._tosr0x_temp_binds:
      self._tosr0x_temp_binds[sender_eui64] = self._tosr_temp.subscribe(lambda x: transmit(sender_eui64, json_dumps({"tosr0x_temp": x}))
...
def register(*args, **kwargs):
  ...
  def rx_callback(x):
    # {'broadcast': False, 'dest_ep': 232, 'sender_eui64': b'\x00\x13\xa2\x00A\xa0n`', 'payload': b'{"command": "test"}', 'sender_nwk': 0, 'source_ep': 232, 'profile': 49413, 'cluster': 17}
    try:
      if x['payload']=="":
        return
      ...
      if hasattr(_commands, 'cmd_' + cmd):
        if args is None:
          response = getattr(_commands, 'cmd_' + cmd)(sender_eui64=x['sender_eui64'])
        elif isinstance(args, dict):
          response = getattr(_commands, 'cmd_' + cmd)(**args, sender_eui64=x['sender_eui64'])
