...
for x in range(3):
  switch_unsubscribe[x] = humidifier_switch[x].subscribe(lambda v: print("switch" + str(x) + " = " + str(v)))
  sensor_unsubscribe[x] = humidifier_sensor[x].subscribe(lambda v: print("sensor" + str(x) + " = " + str(v)))
  available_unsubscribe[x] = humidifier_available[x].subscribe(lambda v: print("available" + str(x) + " = " + str(v)))


humidifier = {x: GenericHygrostat(
...
commands_register(tosr_switch, tosr_temp, humidifier, humidifier_sensor, humidifier_available, humidifier_switch)
